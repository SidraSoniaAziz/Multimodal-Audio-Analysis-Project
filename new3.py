    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                result = model1.transcribe(file_path)
                print(result["text"])
                bert_model1 = Summarizer()
                bert_summary = ''.join(bert_model1(result["text"], min_length=15, max_length=50))
                print("Bert summary: ",bert_summary)
                fb_summary = fb_summarizer(result["text"], max_length=130, min_length=30, do_sample=False)
                print("Facebook bart cnn: ",fb_summary)
                # Assuming fb_summary is a list containing a dictionary
                if isinstance(fb_summary, list) and len(fb_summary) > 0 and isinstance(fb_summary[0], dict):
                    summary_text = fb_summary[0].get("summary_text", "")
                ...
                speakers = []
                for idx, (folder_path, _, audio_files) in enumerate(os.walk(clusters_directory), start=0):
                    if audio_files:
                        for audio_file in audio_files:
                            ...
                            #         gender = "Male"
                            #     age = classes[label]
                            gender = "Female" if gender_result == 0 else "Male"
                            age = predicted_class[0]  # Assuming you only have one predicted class

                            speakers.append({"id": idx, "gender": gender, "age": age})
                # Render the template and send it to the browser
                # return render_template('options.html',transcription_result=result["text"], no_of_speakers=no_of_speakers, speakers=speakers)
                return render_template('options.html',fb_summary=summary_text,bert_summary=bert_summary,transcription_result=result["text"], no_of_speakers=no_of_speakers, speakers=speakers)
