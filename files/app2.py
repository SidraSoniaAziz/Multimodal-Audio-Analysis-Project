...
def options():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                file.save(file_path)
                print("file path before embedding: ", file_path)
                embedding = inference(file_path)
                diarization = pipeline(file_path)
                # print("file path before transcription: ", file_path)
                ...
                # age_label = age_label.max()
                
                
                # print(transcription_result)
                # age_class =label_encoder.inverse_transform(age_label)
                for label in predicted_class:
                    if gender_result == 0:
                        # return render_template("options.html",gender_result = "Female",age_class=label)
                        return render_template("options.html",gender_result = "Female",age_class=label,diarization_speakers=diarization)
                        # return render_template("options.html",gender_result = "Female",age_class=label,transcription_result=transcription_result,diarization_speakers=diarization_speakers)
                    else:
                        # return render_template("options.html",gender_result = "Male",age_class=label)
                        return render_template("options.html",gender_result = "Male",age_class=label,diarization_speakers=diarization)
                        # return render_template("options.html",gender_result = "Male",age_class=label,transcription_result=transcription_result,diarization_speakers=diarization_speakers)
